...
class LSTMModel(nn.Module):
    ...
    def forward(self, x, hidden=None):
        ...
        # Use mean pooling of all hidden states instead of just the last one
        mean_out = torch.mean(out, dim=1)
        return mean_out, (hn, cn)

# Initialize vocabulary and word-to-index mapping
vocab = {}
current_idx = 0
...
def main():
    ...
    hidden_size = 256
    num_layers = 2
    vocab_size = 10000  # Initial vocabulary size
    ...
    llm = HuggingFaceLLM(
        ...
        max_new_tokens=800,
        context_window=2048,
        generate_kwargs={"temperature": 0.3, "top_p": 0.8, "eos_token_id": eos_token_id},
    )
    ...
    while True:
        ...
        # Convert user query and response to tensors
        query_tensor = text_to_tensor(user_query)
        response_tensor = text_to_tensor(str(response))
        
        # Process conversation history through LSTM
        if conversation_history:
            # Process each history item separately
            ...
            for text in conversation_history:
                ...
                context_embedding = mean_out  # Use mean pooled output
        else:
            context_embedding = torch.zeros(1, hidden_size)
        ...
        # Always show the generated answer and request approval
        print("\nGenerated Answer:\n", response)
        approval = input("\nApprove this answer? (y/n): ").strip().lower()
        
        if approval == 'y':
            # Update conversation history only if approved
